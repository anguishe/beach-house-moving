# BRAND.md — Beach House Moving

> The single source of truth for brand voice, messaging, positioning, and identity. All copy written by AI agents must adhere to these guidelines. (No duplicate `BRAND (1).md` exists in this repo — this file is canonical.)

**Canonical domain:** `https://beachhousemoving.xyz`  
**Service-Area Business (SAB):** Public NAP = Name + Phone + Service Area only. The street address in `content.ts` is internal (schema locality / GBP consistency) and is **never displayed** on the site.

---

## Brand Essence

**Who we are:** A locally owned, fully licensed and insured moving & storage company rooted in the Florida Panhandle. Established 2025. Three-truck fleet. We treat every move like it's our own.

**What we do:** Full-service moving — packing, loading, transportation, unloading, unpacking — for homes and businesses across Walton, Okaloosa, and Bay Counties and beyond.

**Why we exist:** Moving is stressful. We remove that stress with professionalism, care, and local expertise that no national chain can replicate.

---

## Positioning Statement

> "Beach House Moving is the Florida Panhandle's premier locally owned moving company — delivering the highest level of professionalism, safety, and personal care for every move, every time."

---

## Brand Values

| Value | What It Means in Practice |
|---|---|
| **Local Pride** | We live here, we work here, we care about this community |
| **Professionalism** | Fully licensed, insured, uniformed, on-time, communicative |
| **Safety** | Your belongings are treated as if they were our own |
| **Transparency** | Free estimates, no hidden fees, honest timelines |
| **Reliability** | We show up. Every time. |

---

## Target Audience

### Primary: Residential Homeowners & Renters
- Families moving between homes in the 30A / Destin / Fort Walton area
- Retirees relocating to the Panhandle (high-value, trust-sensitive)
- Vacation rental owners moving furnishings between properties
- Military families (Eglin AFB, Hurlburt Field) — frequent movers, value reliability

### Secondary: Small Business Owners
- Local restaurants, boutiques, offices relocating along the Emerald Coast
- Short-term rental (Airbnb/VRBO) operators needing delivery & staging help

### Psychographic Profile
- Values local over corporate
- Willing to pay more for trustworthiness
- Makes decisions based on reviews and referrals
- Anxious about moving — wants reassurance upfront

---

## Voice & Tone

### The Brand Voice Is:
- **Warm** — friendly, approachable, like a trusted neighbor
- **Confident** — we know what we're doing; no hedging
- **Professional** — this is a licensed business, not a couple of guys with a truck
- **Local** — mentions of the area, the coast, the community feel authentic

### The Brand Voice Is NOT:
- Corporate or stiff
- Overly casual or slangy
- Pushy or salesy
- Vague or full of filler words

---

## Messaging Framework

### Primary Headline Options
- "Your Move, Our Mission."
- "Moving Made Easy on the Emerald Coast."
- "The Panhandle's Most Trusted Movers."
- "Stress-Free Moving Starts Here."

**Approved headline (use this unless directed otherwise):**
> "Your Move, Our Mission."

### Tagline
> "Simply A Better Choice."

The tagline is the brand signoff (footer, closing brand moments). Do not substitute the credentials line for it.

### Credentials Line (Trust Signal)
> "Licensed & Insured · Locally Owned & Operated"

The credentials line is the trust signal — use it above the fold and in hero trust badges, not as the tagline.

### Hero Subheadline
> "Beach House Moving serves Walton, Okaloosa, and Bay Counties with professional packing, loading, and transportation services. Licensed, insured, and ready when you are."

---

## Copy Rules

1. **Lead with trust signals** — "Licensed & Insured" and **Florida Mover Reg. #IM4125** appear above the fold on every page
2. **Use "you" and "your"** — customer-first language, not company-first
3. **Be specific about geography** — mention county and city names; it builds local credibility
4. **Avoid moving clichés** — do not write "we treat your belongings like our own" (overused). Find fresh ways to say it.
5. **Never fabricate numbers** — do not invent review counts, years in business, move counts, or awards. Use only verified facts (established 2025, 3-truck fleet, 24/7 availability).
6. **Primary CTA is quote-first** — "Get a Free Quote" (coral button) is the primary above-the-fold action sitewide; phone is secondary
7. **CTA copy is action-oriented** — "Get a Free Quote", "Call Us Now"
8. **Keep paragraphs short** — max 3 sentences. Panhandle customers are reading on mobile.
9. **No exclamation points** — calm, confident tone. Periods only.
10. **Testimonials are gated** — never render testimonials while `FLAGS.SHOW_TESTIMONIALS` is `false`; wait for verified Google/Facebook reviews

---

## Service Descriptions (Approved Copy)

### Packing & Unpacking
> "We handle the boxes, the bubble wrap, and the breakables. Our team packs your home carefully and efficiently — and unpacks it just as thoughtfully on the other end."

### Residential Moving
> "Whether you're moving across the street or across the county, we manage every detail of your residential move with care, speed, and zero drama."

### Local Moving
> "Serving the greater 30A and Emerald Coast area, our local moving crews know these roads, these neighborhoods, and how to get your belongings there safely."

### Long-Distance Moving
> "Moving beyond the Panhandle? We offer long-distance moving services with the same professionalism and care you'd expect for a local job."

### Storage
> "Need a safe place for your belongings between moves or during renovations? We offer secure storage solutions to fit your timeline."

### Delivery
> "Furniture delivery, appliance placement, single-item transport — we handle it all with the same professionalism as a full move."

---

## What to Always Include on the Site

- ✅ "Licensed & Insured" + **Florida Mover Reg. #IM4125** — every page, above the fold
- ✅ Primary CTA **"Get a Free Quote"** — coral button, above the fold sitewide
- ✅ Phone number `(850) 842-1962` — clickable, visible (secondary to quote CTA)
- ✅ "Locally Owned & Operated" — trust signal
- ✅ Service area counties (Walton, Okaloosa, Bay) + long-distance
- ✅ Free estimate offer
- ✅ "Available 24/7" — open seven days a week
- ❌ Street address — never displayed (SAB)
- ❌ Unverified testimonials — hidden until `FLAGS.SHOW_TESTIMONIALS` is enabled

---

## Competitor Landscape (for reference)

| Competitor | Strength | Our Advantage |
|---|---|---|
| Emerald Coast Moving & Storage | Google Guaranteed, 24/7 | More personal, local story |
| Summit Movers Inc. | 12 years in business | Modern website, fresher brand |
| National chains (Allied, etc.) | Brand recognition | Local trust, no subcontracting |

---

## Brand Do's and Don'ts

| ✅ Do | ❌ Don't |
|---|---|
| Use "Panhandle" and "Emerald Coast" | Use generic "Florida" without specificity |
| Reference 30A, Destin, Fort Walton | Use corporate-speak ("leverage", "solutions") |
| Write like a trusted local business | Write like an SEO content farm |
| Emphasize peace of mind | Overload with bullet points of features |
| Show personality and warmth | Be cold or transactional |
