...
export async function POST(req: NextRequest) {
  try {
    ...
    await backupLead('quote', { ...data, company: undefined })
    ...
    const { error: sendError } = await resend.emails.send({
      ...
      subject: `New Quote Request — ${data.moveType.replace(/[\r\n]/g, ' ')} | Beach House Moving`,
      html: `<!DOCTYPE html>
<html>
...
<body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; color: #1B2B4B;">
  ...
  <div style="padding: 24px; background: #f9f9f9; border: 1px solid #e2e8f0;">
    <table style="width: 100%; border-collapse: collapse;">
      ...
      <tr><td style="padding: 8px 0; color: #718096; font-size: 13px;">Move Type</td><td style="padding: 8px 0;">${e(data.moveType)}</td></tr>
      <tr><td style="padding: 8px 0; color: #718096; font-size: 13px;">Home Size</td><td style="padding: 8px 0;">${e(data.homeSize) || '—'}</td></tr>
      <tr><td style="padding: 8px 0; color: #718096; font-size: 13px;">Move Date</td><td style="padding: 8px 0;">${e(data.moveDate) || '—'}</td></tr>
      <tr><td style="padding: 8px 0; color: #718096; font-size: 13px;">Moving From</td><td style="padding: 8px 0;">${e(data.moveFrom)}</td></tr>
      <tr><td style="padding: 8px 0; color: #718096; font-size: 13px;">Moving To</td><td style="padding: 8px 0;">${e(data.moveTo)}</td></tr>
    ...
    ${data.notes ? `<div style="margin-top: 16px; padding: 12px; background: white; border-left: 3px solid #E85D3D; border-radius: 0 4px 4px 0;"><p style="margin: 0; color: #718096; font-size: 12px; text-transform: uppercase; letter-spacing: 0.05em;">Notes</p><p style="margin: 8px 0 0;">${e(data.notes)}</p></div>` : ''}
  </div>
  <div style="padding: 16px 24px; background: #f9f9f9; border: 1px solid #e2e8f0; border-top: 0;">
    ...
    <table style="width: 100%; border-collapse: collapse;">${sourceRows(data.source, data.heardAbout)}</table>
  </div>
